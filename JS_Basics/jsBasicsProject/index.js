...
while (i < questions.length) {
    ...
    switch(i) {
        ...
        case 1: // check the input for last_name
            validAnswer = (typeof answer === 'string' && answer.trim() !== '');
            if (validAnswer) {
                if (i === 0) { 
        ...
        case 4: // check the input for gender_interest
            validAnswer = (typeof answer === 'string' && answer.trim() !== '' && answer === 'F' || answer === 'M' || answer === 'X')
            switch(answer) {
                case 'F':
                case 'M':
                case 'X':
...
// Looking for a match for our new profile
